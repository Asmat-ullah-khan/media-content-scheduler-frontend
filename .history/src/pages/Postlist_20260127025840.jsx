  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      if (response.data.status === "success") {
        ...
        alert("Post created successfully!"); // Ensure alert is visible
      }
    } catch (err) {
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      if (response.data.status === "success") {
        ...
        fetchPosts(); // Refresh list
        alert("Post updated successfully!"); // Ensure alert visible
      }
    } catch (err) {
    ...
    try {
      ...
      if (response.data.status === "success") {
        ...
        setTotalPages((prev) =>
          Math.max(
            1,
            Math.ceil((prev * postsPerPage - postsPerPage) / postsPerPage),
          ),
        );
        if (currentPage > 1 && posts.length === 1) {
          setCurrentPage(currentPage - 1); // Go to previous page if last post on page
        }
